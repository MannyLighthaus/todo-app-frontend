...
async function handleTodo(event: Event) {
  ...
  if (
    ...
    const existingTodoId = (event.target as HTMLElement).dataset.id;
    ...
    if (existingTodoId) {
      ...
      try {
        ...
        (event.target as HTMLElement).removeAttribute("data-id");
      } catch (error) {
...
async function deleteTodo(event: Event) {
  ...
  if ((event.target as HTMLElement).closest(".delete-icon")) {
    ...
    const todoItem = (event.target as HTMLElement).closest(".todo");
