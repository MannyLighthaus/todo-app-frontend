...
// Move task to completed or incomplete list after checkbox click
document.addEventListener("change", async function (event) {
  if (!event.target.classList.contains("checkbox")) return;

  const checkbox = event.target;
  const todoItem = checkbox.closest(".todo");
  const todoId = todoItem.dataset.id; // Retrieve the todo ID

  const newState = checkbox.checked ? STATES.COMPLETED : STATES.INCOMPLETE; // Toggle state
  ...
  try {
    ...
    console.log("Todo state updated:", updatedTodo);

    // Move item between lists in the UI
    ...
    // Adjust visibility of completed/incomplete sections
    completedMainContentElement.style.display =
      completedTodoListElement.children.length > 0 ? "block" : "none";

    incompleteMainContentElement.classList.toggle(
      "incomplete-empty-display",
      incompleteTodoListElement.children.length === 0
    );
