...
// get the update-button element by class
...
function switchPage(isHomePage) {
  if (isHomePage) {
    ...
    // });
    console.log("enter out");
    updatebtn.removeEventListener("click", submitEdit);
...
// Moving to completed list after click on checkbox
...
function renderTodos(todos) {
  const incompleteTodoList = document.getElementById("incomplete-todo-list"); // Ensure this exists in your HTML
  incompleteTodoList.innerHTML = ""; // Clear existing content

  const completedTodoList = document.getElementById("completed-todo-list"); // Ensure this exists in your HTML
  ...
  todos.forEach((todo) => {
    ...
    checkbox.addEventListener("change", async () => {
      // Toggle the state of the todo item
      todo.state = checkbox.checked ? 2 : 1; // 2 = Completed, 1 = Incomplete

      // Re-render the todos to reflect the changes
      // renderTodos(todos);
...
async function saveTodo(event) {
  ...
  if (
    ...
    try {
      console.log("Sending Todo:", newTodoItem); // Debugging
      ...
      todos.unshift(savedTodo.todo); // add new todo at the beginning
    ...
    if (todoItem) {
      ...
      if (isconfirmed) {
        ...
        try {
          ...
          todos = todos.filter((todo) => todo._id !== idToDelete);
...
async function editTodo(event) {
  ...
  const clickedLi = target.closest("li"); // Find the closest <li>
  ...
  // Store the Todo Id inside the updateBtn dataset
  updatebtn.dataset.id = retrievedTodoId;
  ...
  console.log("Calling Event Listener now");
  updatebtn.addEventListener("click", submitEdit);
}
...
async function submitEdit(event) {
  ...
  await updateTodo(event, existingTodoId);
...
async function updateTodo(event, id) {
  ...
  const editedTodoItem = {
    title: textFieldValue,
    category: selectedCategoryItem, // Ensure category object is sent
    date: dateFieldValue,
    time: timeFieldValue,
    notes: textAreaField,
    state: 1,
  ...
  try {
    ...
    // const editedTodoElement = document.querySelector(
    //   `[data-id="${updatedTodo._id}"]`
    // );

    // console.log(editedTodoElement);

    // editedTodoElement.dataset.date = updatedTodo.date;
    // editedTodoElement.dataset.time = updatedTodo.time;
    // editedTodoElement.dataset.notes = updatedTodo.notes;
    // editedTodoElement.dataset.category = updatedTodo.category;

    // editedTodoElement.innerHTML = `
    //   <div class="icon-text-container">
    //     <div data-id="${updatedTodo.category.id}"
    //          style="background-color:${updatedTodo.category.color}"
    //          class="category-icon-container">
    //       ${updatedTodo.category.icon}
    //     </div>
    //     <p class="todo-text">${updatedTodo.title}</p>
    //   </div>
    //   <div class="checkbox-delete-container">
    //     <div class="checkbox-container">
    //       <input type="checkbox" class="checkbox" />
    ...
    //         <g clip-path="url(#clip0_83_362)">
    //           <path d="M20.25 5.25H3.75" stroke="#999999" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
    //           <path d="M9.75 9.75V15.75" stroke="#999999" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
    //           <path d="M14.25 9.75V15.75" stroke="#999999" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
    //           <path d="M18.75 5.25V19.5C18.75 19.6989 18.671 19.8897 18.5303 20.0303C18.3897 20.171 18.1989 20.25 18 20.25H6C5.80109 20.25 5.61032 20.171 5.46967 20.0303C5.32902 19.8897 5.25 19.6989 5.25 19.5V5.25" stroke="#999999" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
    //           <path d="M15.75 5.25V3.75C15.75 3.35218 15.592 2.97064 15.3107 2.68934C15.0294 2.40804 14.6478 2.25 14.25 2.25H9.75C9.35218 2.25 8.97064 2.40804 8.68934 2.68934C8.40804 2.97064 8.25 3.35218 8.25 3.75V5.25" stroke="#999999" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"/>
    //         </g>
    //         <defs>
    //           <clipPath id="clip0_83_362">
    //             <rect width="24" height="24" fill="white"/>
    ...
    //move the updated todo to the top of the incomplete list
    // incompleteTodoListElement.prepend(editedTodoElement);
...
//     const checkbox = li.querySelector(".checkbox");
//     checkbox.addEventListener("change", async () => {
//       const newState = checkbox.checked ? 2 : 1; // 2 = Completed, 1 = Incomplete

//       try {
//         const response = await fetch(
//           `http://localhost:000/todos/${todo._id}`,
//           {
//             method: "PUT",
//             headers: {
//               "Content-Type": "application/json",
...
//         // Update the local state and re-render the todos
//         todo.state = newState;
//         renderTodos(todos);
//       } catch (error) {
